...
const ThemeIcon: Record<ThemeType['name'], React.ReactNode> = {
  System: (
...
interface IThemeOptionProps {
  ...
  checked: boolean;
  onClick: (name: ThemeType['name']) => void;
}
...
export const ThemeSwitcher: FC = () => {
  ...
  return (
    <div
      className='theme-transition relative z-0 inline-grid grid-cols-3 gap-0.5 rounded-full bg-gray-950/5 p-0.75 text-gray-950 dark:bg-white/10'
      role='radiogroup'
    >
      {themeList.map(({ name }) => {
        return (
          <ThemeOption key={name} name={name} icon={ThemeIcon[name]} checked={theme.name === name} onClick={setTheme} />
        );
      })}
    </div>
  );
